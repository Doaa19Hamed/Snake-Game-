...
class Square:
    def _init_(self, pos, face, food=False):
        self.surface = face
        self.food = food
        self.is_tail = False
        self.pos = pos
        self.dir = [-1, 0]             #SET DIRECTIONS
        if self.food:
            self.dir = [0, 0]

    def paint(self, color=SNAKE_CLR):
        Z, F = self.pos[0], self.pos[1]
        SS , GS = SQUARE_SIZE, GAP_SIZE         # WE SET SS THE SIZE OF SQUARE-SIZE , GS THE GAP-SIZE
        if self.dir == [-1, 0]:
            if self.is_tail:
                pygame.draw.rect(self.surface, color,(Z* SS+ GS, F * SS + GS , SS - 2 * GS, SS - 2 * GS))
            else:
                pygame.draw.rect(self.surface, color,(Z * SS + GS, F * SS + GS, SS, SS - 2 * GS))
        if self.dir == [1, 0]:
            if self.is_tail:
                pygame.draw.rect(self.surface, color,(Z * SS + GS, F * SS + GS, SS - 2 * GS, SS - 2 * GS))
            else:
        ...
        if self.dir == [0, 1]:
            if self.is_tail:
                ...
                pygame.draw.rect(self.surface, color,(Z * SS + GS, F * SS - GS, SS- 2 * GS, SS))
        if self.dir == [0, -1]:
            if self.is_tail:
                pygame.draw.rect(self.surface, color,(Z * SS + GS, F * SS + GS, SS - 2 * GS, SS - 2 * GS))
            else:
...
class Snake:
    ...
    def is_position_free(self, position): # checks if the position square free or not
        if position[0] >= ROWS or position[0] < 0 or position[1] >= ROWS or position[1] < 0:
            return False
        for sqr in self.squares:
            if sqr.pos == position:# the given square position could be occupied by another square
                return False
        return True
    ...
    def bfs(self, s, e):  # the shortest path : start and end
        queue = [s]
        ...
        path = list()
        parent_node = e # end node -->  find the parent node
    ...
    def get_path_to_tail(self): # return the path from the head of a list to its tail.
        ...
        self.add_square() # to make a complete path from the head to tail
        return path
    ...
    def update(self):
        ...
        if self.score == ROWS * ROWS - INITIAL_SNAKE_LENGTH:  # win the game
            ...
            pygame.time.wait(1000 * AFTER_WIN)
            return 1
        self.total_moves += 1
